...
namespace Implementation.Validators
{
    public class RegisterUserValidator : AbstractValidator<RegisterDto>
    {
        ...
        public RegisterUserValidator(ShopDbContext _context)
        {
            RuleFor(x => x.Email)
                .Cascade(CascadeMode.Stop)
                .NotEmpty().WithMessage("Email je obavezan podatak.")
                .EmailAddress().WithMessage("Email nije ispravnog formata.")
                .Must(EmailAlreadyExists).WithMessage("Email adresa {PropertyValue} je već u upotrebi.");
            ...
            _shopDbContext = _context;
        }


        public bool EmailAlreadyExists(string Email)
        {
            return !_shopDbContext.Users.Any(u => u.Email == Email);
        }
    }
}
